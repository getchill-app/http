...
func (c *Client) OrgCreateVault(ctx context.Context, org *keys.EdX25519Key, vault *keys.EdX25519Key) (*api.Vault, error) {
	ek, err := encryptKey(vault, org.ID())
	if err != nil {
		return nil, err
	}
	path := dstore.Path("org", org.ID(), "vault")
	create := &api.OrgVaultCreateRequest{KID: vault.ID().String(), EncyptedKey: ek}
	body, err := json.Marshal(create)
	if err != nil {
		return nil, err
	}
	resp, err := c.Request(ctx, &client.Request{Method: "PUT", Path: path, Body: body, Key: org})
	if err != nil {
		return nil, err
	}
...
func (c *Client) OrgInvite(ctx context.Context, org *keys.EdX25519Key, invite keys.ID, invitedBy *keys.EdX25519Key) error {
	...
	auths := []http.AuthHeader{
		...
		{Header: "Authorization-Account", Key: invitedBy},
	}
...
func (c *Client) OrgAccountInvites(ctx context.Context, account *keys.EdX25519Key) ([]*api.OrgInvite, error) {
	path := dstore.Path("account", account.ID(), "org-invites")
...
func (c *Client) OrgInviteAccept(ctx context.Context, account *keys.EdX25519Key, org *keys.EdX25519Key) error {
	path := dstore.Path("account", account.ID(), "org", org, "invite/accept")

	auths := []http.AuthHeader{
		{Header: "Authorization-Org", Key: org},
		{Header: "Authorization-Account", Key: account},
	}
	if _, err := c.Request(ctx, &client.Request{Method: "POST", Path: path, Auths: auths}); err != nil {
		return err
	}
	return nil
}
